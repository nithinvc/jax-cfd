...
def generate_output_file_name(args):
    file_name = f"ns_{args.resolution}x{args.resolution}_visc_{args.viscosity}.npy"
...
def main():
    ...
    parser.add_argument("--simulation_time", type=float, default=0.001)
    parser.add_argument(
        "--save_dt",
        type=float,
        default=1.0,
    ...
    # ensure output dir exists otherwise make it
    ...
    inner_steps = burn_in_time // stable_dt
    outer_steps = 1
    ...
    def generate_solution_template(key, trajectory_fn):
        ...
        print('spectral_trajectory.shape', spectral_trajectory.shape)
    ...
    for batch_number in tqdm(range(num_batches)):
        ...
        jnp.save(generate_output_file_name(args), batch_trajectories)
